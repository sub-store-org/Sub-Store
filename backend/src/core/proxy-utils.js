...
const { AND, FULL } = require('../utils/logical');
const { getFlag } = require('../utils/geo');
...
const PROXY_PREPROCESSORS = (function() {
	...
	function Base64Encoded() {
		...
		const keys = [ 'dm1lc3M', 'c3NyOi8v', 'dHJvamFu', 'c3M6Ly', 'c3NkOi8v', 'c2hhZG93', 'aHR0c' ];
	...
	function Clash() {
		...
		const parse = function(raw) {
			// Clash YAML format
			const proxies = YAML.safeLoad(raw).proxies;
		...
		const parse = function(raw) {
			...
			for (let i = 0; i < servers.length; i++) {
				...
				let plugin = server.plugin_options
					? '/?plugin=' + encodeURIComponent(server.plugin + ';' + server.plugin_options)
					: '';
...
const PROXY_PARSERS = (function() {
	...
	function URI_SSR() {
		...
		const parse = (line) => {
			...
			proxy = {
				...
				'protocol-param': Base64.decode(other_params.protoparam || '').replace(/\s/g, ''),
				'obfs-param': Base64.decode(other_params.obfsparam || '').replace(/\s/g, '')
			};
			return proxy;
		};
		...
		const parse = (line) => {
			...
			if (/=\s*vmess/.test(content)) {
				...
				if (params.obfs === 'ws' || params.obfs === 'wss') {
					proxy.network = 'ws';
					proxy['ws-opts'].path = (params['obfs-path'] || '"/"').match(/^"(.*)"$/)[1];
			...
			if (proxy.tls) {
				if (typeof params['skip-cert-verify'] !== 'undefined') {
					proxy['skip-cert-verify'] =
						params['skip-cert-verify'] === true || params['skip-cert-verify'] === '1';
				}
	...
	function Surge_Http() {
		...
		const test = (line) => {
			return /^.*=\s*http/.test(line.split(',')[0]) && !Loon_Http().test(line);
		};
		const parse = (line) => {
			const params = getSurgeParams(line);
			const proxy = {
			...
			switch (proxy.type) {
				case 'ss':
					...
					if (proxy.plugin === 'v2ray-plugin') {
						...
						obfs_opts = `,obfs=${tls ? 'wss' : 'ws'}${host ? ',obfs-host=' + host : ''}${path
							? ',obfs-uri=' + path
							: ''}`;
					}
					return `shadowsocks=${proxy.server}:${proxy.port},method=${proxy.cipher},password=${proxy.password}${obfs_opts}${proxy.tfo
						? ',fast-open=true'
						: ',fast-open=false'}${proxy.udp ? ',udp-relay=true' : ',udp-relay=false'},tag=${proxy.name}`;
				case 'ssr':
					return `shadowsocks=${proxy.server}:${proxy.port},method=${proxy.cipher},password=${proxy.password},ssr-protocol=${proxy.protocol}${proxy[
						'protocol-param'
					]
						? ',ssr-protocol-param=' + proxy['protocol-param']
						: ''}${proxy.obfs ? ',obfs=' + proxy.obfs : ''}${proxy['obfs-param']
						? ',obfs-host=' + proxy['obfs-param']
						: ''},fast-open=${proxy.tfo || false}${proxy.udp
						? ',udp-relay=true'
				...
				case 'vmess':
					...
					if (proxy.network === 'ws') {
						...
						if (proxy.tls) {
							// ws-tls
							obfs_opts = `,obfs=wss${proxy.sni ? ',obfs-host=' + proxy.sni : ''}${proxy['ws-opts'].path
						...
						// tcp
						if (proxy.tls) {
							obfs_opts = `,obfs=over-tls${proxy.sni
								? ',obfs-host=' + proxy.sni
								: ''},tls-verification=${proxy['skip-cert-verify'] ? 'false' : 'true'}`;
						}
					}
					let result = `vmess=${proxy.server}:${proxy.port},method=${proxy.cipher === 'auto'
						? 'none'
						: proxy.cipher},password=${proxy.uuid}${obfs_opts},fast-open=${proxy.tfo || false}${proxy.udp
						? ',udp-relay=true'
						: ',udp-relay=false'}`;
					...
					return result;
				case 'trojan':
					return `trojan=${proxy.server}:${proxy.port},password=${proxy.password}${proxy.sni
						? ',tls-host=' + proxy.sni
						: ''},over-tls=true,tls-verification=${proxy['skip-cert-verify']
						? 'false'
						: 'true'},fast-open=${proxy.tfo || false}${proxy.udp
						? ',udp-relay=true'
						: ',udp-relay=false'},tag=${proxy.name}`;
				case 'http':
					tls_opts = '';
					if (proxy.tls) {
						tls_opts = `,over-tls=true,tls-verification=${proxy['skip-cert-verify']
							? 'false'
							: 'true'}${proxy.sni ? ',tls-host=' + proxy.sni : ''}`;
					}
					return `http=${proxy.server}:${proxy.port},username=${proxy.username},password=${proxy.password}${tls_opts},fast-open=${proxy.tfo ||
						false},tag=${proxy.name}`;
			}
	...
	function Loon_Producer() {
		...
		const produce = (proxy) => {
			...
			switch (proxy.type) {
				...
				case 'ssr':
					return `${proxy.name}=shadowsocksr,${proxy.server},${proxy.port},${proxy.cipher},"${proxy.password}",${proxy.protocol},{${proxy[
						'protocol-param'
					] || ''}},${proxy.obfs},{${proxy['obfs-param'] || ''}}${udp_opts}${tfo_opts}`;
				case 'vmess':
					obfs_opts = '';
					if (proxy.network === 'ws') {
					...
					if (proxy.tls) {
						obfs_opts += `${proxy.sni ? ',tls-name:' + proxy.sni : ''},skip-cert-verify:${proxy[
							'skip-cert-verify'
						] || 'false'}`;
					}
					let result = `${proxy.name}=vmess,${proxy.server},${proxy.port},${proxy.cipher === 'auto'
						? 'none'
						: proxy.cipher},"${proxy.uuid}",over-tls:${proxy.tls || 'false'}${obfs_opts}`;
					if (proxy.alterId === 0) proxy['vmess-aead'] = true;
					if (typeof proxy['vmess-aead'] !== 'undefined') {
					...
					return result;
				case 'trojan':
					return `${proxy.name}=trojan,${proxy.server},${proxy.port},"${proxy.password}"${proxy.sni
						? ',tls-name:' + proxy.sni
						: ''},skip-cert-verify:${proxy['skip-cert-verify'] || 'false'}${udp_opts}`;
				case 'http':
					tls_opts = '';
					const base = `${proxy.name}=${proxy.tls
						? 'http'
						: 'https'},${proxy.server},${proxy.port},${proxy.username || ''},${proxy.password || ''}`;
					if (proxy.tls) {
						// https
						tls_opts = `${proxy.sni ? ',tls-name:' + proxy.sni : ''},skip-cert-verify:${proxy[
							'skip-cert-verify'
						]}`;
		...
		const produce = (proxy) => {
			...
			switch (proxy.type) {
				case 'ss':
					...
					result = `${proxy.name}=ss,${proxy.server}, ${proxy.port},encrypt-method=${proxy.cipher},password=${proxy.password}${obfs_opts},tfo=${proxy.tfo ||
						'false'},udp-relay=${proxy.udp || 'false'}`;
					break;
				case 'vmess':
					tls_opts = '';
					result = `${proxy.name}=vmess,${proxy.server},${proxy.port},username=${proxy.uuid},tls=${proxy.tls ||
						'false'},tfo=${proxy.tfo || 'false'}`;
					...
					if (proxy.network === 'ws') {
						...
						const wsHeaders = Object.entries(proxy['ws-opts'].headers)
							...
							.join('|');
						result += `,ws=true${path ? ',ws-path=' + path : ''}${wsHeaders
					...
					break;
				case 'trojan':
					result = `${proxy.name}=trojan,${proxy.server},${proxy.port},password=${proxy.password}${typeof proxy[
						'skip-cert-verify'
					] !== 'undefined'
						? ',skip-cert-verify=' + proxy['skip-cert-verify']
						: ''}${proxy.sni ? ',sni=' + proxy.sni : ''},tfo=${proxy.tfo ||
						'false'},udp-relay=${proxy.udp || 'false'}`;
					break;
				case 'http':
					tls_opts = ', tls=false';
					...
					result = `${proxy.name}=http, ${proxy.server}, ${proxy.port}${proxy.username
						? ',username=' + proxy.username
						: ''}${proxy.password ? ',password=' + proxy.password : ''}${tls_opts},tfo=${proxy.tfo ||
						'false'}`;
					break;
				default:
					throw new Error(`Platform ${targetPlatform} does not support proxy type: ${proxy.type}`);
		...
		const produce = (proxy) => {
			...
			switch (proxy.type) {
				case 'ss':
					...
					result = `ss://${Base64.encode(userinfo)}@${proxy.server}:${proxy.port}/`;
					if (proxy.plugin) {
						result += '?plugin=';
						...
						switch (proxy.plugin) {
							...
							case 'v2ray-plugin':
								result += encodeURIComponent(
									`v2ray-plugin;obfs=${opts.mode}${opts.host
										? ';obfs-host' + opts.host
										: ''}${opts.tls ? ';tls' : ''}`
								);
								break;
					...
					break;
				case 'ssr':
					result = `${proxy.server}:${proxy.port}:${proxy.protocol}:${proxy.cipher}:${proxy.obfs}:${Base64.encode(
						proxy.password
					)}/`;
				...
				case 'vmess':
					...
					break;
				case 'trojan':
					result = `trojan://${proxy.password}@${proxy.server}:${proxy.port}#${encodeURIComponent(
						proxy.name
					)}`;
					break;
				default:
			...
			if (item.type.indexOf('Script') !== -1) {
				...
				if (mode === 'link') {
					...
					try {
						...
						$.error(`Error when downloading remote script: ${item.args.content}.\n Reason: ${err}`);
...
function ApplyProcessor(processor, objs) {
