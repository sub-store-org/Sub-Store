...
const NATIVE_COMMON_FIELDS = [
    ...
    // Metadata generated by the built-in domain resolver, never native options.
    '_IP',
    '_IPv4',
    '_IPv6',
    '_IP4P',
...
function rejectUnknownNativeOptions(proxy) {
    ...
    const unsupported = Object.keys(proxy).filter(
        (key) =>
            ...
            !(proxy.type === 'ss' && key === 'tls' && proxy[key] === false) &&
            !(['_h2', 'skip-cert-verify', 'tfo', 'fast-open', 'disable-sni', 'reduce-rtt'].includes(key) && proxy[key] === false) &&
            hasConfiguredNativeValue(proxy[key]),
    );
    if (unsupported.length > 0) {
        throw new Error(
...
function produceNativeVless(proxy) {
    ...
    if (proxy.encryption != null && proxy.encryption !== 'none') {
        throw new Error('Unsupported Shadowrocket native VLESS encryption');
    }
...
function produceNativeSocks5(proxy) {
    ...
    if (proxy.tls) {
        ...
        rejectNativeOptions(proxy, 'SOCKS5', ['skip-cert-verify', 'sni', 'servername']);
    }
...
function produceNativeHysteria(proxy) {
    ...
    appendNativeOption(parts, 'auth', proxy['auth-str']);
